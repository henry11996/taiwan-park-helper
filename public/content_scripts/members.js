...
const injectSelectorIds = ['member', 'collapseOne', 'collapseTwo']
...
function injectSelectors() {
  injectSelectorIds.forEach((id) => {
    ...
    if (id == 'member') {
      ...
      // 其他申請人資料
      $(autoSelector(members, 'inject-selector')).insertAfter($(`#${id} .checkbox`).first())
      reRegisterEvent()
    }
  })
...
if ($("span.highlight-index").length == 2) {
  //等待元素出現
  const block = $("<div></div>")
    .addClass("panel")
